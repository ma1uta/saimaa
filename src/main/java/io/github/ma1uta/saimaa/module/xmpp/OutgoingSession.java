...
import io.github.ma1uta.saimaa.module.xmpp.dialback.Dialback;
import io.github.ma1uta.saimaa.module.xmpp.dialback.ServerDialback;
import io.github.ma1uta.saimaa.module.xmpp.netty.XmppClientInitializer;
...
public class OutgoingSession extends Session {
    ...
    public OutgoingSession(XmppModule xmppModule, String domain, boolean dialback) throws JAXBException {
        super(xmppModule);
    ...
    public boolean handleStream(Object streamElement) throws XmppException {
        ...
        if (!initialized.get()) {
            switch (getXmppModule().dialback().negotiateOutgoing(this, streamElement)) {
                case IN_PROCESS:
                case FAILED:
                    return false;
                case SUCCESS:
    ...
    public void handshake() {
        StreamHeader header = isDialbackEnabled()
            ...
            Jid.ofDomain(getXmppModule().getConfig().getDomain()),
     ...
     */
    public void connect() {
        getXmppModule().getSrvNameResolver().resolve(getDomain(),
            (hostname, port) -> NettyBuilder.createClient(hostname, port, new XmppClientInitializer(getXmppModule(), this), null));
    }
}
